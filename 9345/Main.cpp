...
int main() {
	...
	while (t--) {
		...
		for (int i = high; i < high  + n; i++) {
			tree_min[i] = tree_max[i] = i - high + 1;
		}
		//setup
		...
		for (int cur = 1; cur <= m; cur++) {
			...
			s++; e++;
			if (mode) {
				if (find_min(1, 1, high, s, e) == s&&find_max(1, 1, high, s, e) == e)printf("YES\n");
				else printf("NO\n");
			}
			else {
				int temp = tree_max[high + s - 1];
				tree_max[high + s - 1] = tree_max[high + e - 1];
				tree_min[high + s - 1] = tree_max[high + e - 1];
				tree_max[high + e - 1] = temp;
				tree_min[high + e - 1] = temp;
				int i = (high + s - 1) >> 1;
				while (i) {
					tree_max[i] = max(tree_max[i << 1], tree_max[(i << 1) + 1]);
					tree_min[i] = min(tree_min[i << 1], tree_min[(i << 1) + 1]);
